function fish_prompt
  ...
  set -l current_kube_context (kubectl config current-context)

  ##
  ## Line 1
  ##
  echo -n $hostcolor'╭─'$hotpink$current_user$white' at '$orange$__fish_prompt_hostname$white$turquoise
  __fish_git_prompt " (%s)"
  echo -n "$purple ($current_kube_context)"
  echo
  ...
  echo -n "$limegreen"[(pwd | sed "s=$HOME=~=")]
